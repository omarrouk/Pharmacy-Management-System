...
test("drugs list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/drugs`,
  );
  const body = await response.json();
...
test("batches list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/batches`,
  );
  const body = await response.json();
...
test("inventory list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/inventory?locationType=warehouse&locationId=aaaaaaaaaaaaaaaaaaaaaaaa`,
  );
  const body = await response.json();
...
test("stock movements list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/stock-movements`,
  );
  const body = await response.json();
...
test("suppliers list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/suppliers`,
  );
  const body = await response.json();
...
test("purchase requests list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/purchase-requests`,
  );
  const body = await response.json();
...
test("payment methods list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/payment-methods`,
  );
  const body = await response.json();
...
test("sales invoices list requires authentication", async (t) => {
  const server = await startTestServer();
  t.after(() => server.close());

  const response = await fetch(
    `http://127.0.0.1:${server.address().port}/api/v1/sales-invoices`,
  );
  const body = await response.json();
